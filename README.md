...
| Fáze | Co se děje |
|---|---|
| **0-1.5 min** | SCD41 hlásí **+2.5 °C** nad konečnou hodnotou, vlhkost **-14 %**, CO₂ +56 ppm. Prudce klesá. Není to teplota okolí, ale chování senzoru po startu měření - viz Known Issues. |
| **1.5-3 min** | Projde správnou hodnotou a **podstřelí o 0.5 °C** dolů. |
| **3-15 min** | Teprve tady se zahřívá krabička. Plynulý návrat nahoru, časová konstanta ~3 min (SCD41) / ~6 min (BMP180). Teplotní offset totiž platí jen v tepelné rovnováze - dokud se box neprohřeje, je hodnota systematicky vedle, i když vypadá stabilně. Datasheet proto žádné číslo nedává, jen říká "in thermal equilibrium". |

Doby ustálení z toho záznamu: teplota SCD41 do ±0.1 °C od **12 min**, BMP180 od **9 min**, vlhkost do ±0.5 % od **11 min**, CO₂ do ±15 ppm od 14 min.
...
- **Vlnovka.** Prvních **20 min** po studeném startu se na obou teplotních stránkách i na vlhkosti zobrazuje `~` před hodnotou (`~25.1 C`) = "ještě se to zahřívá, neber to doslova". Vlhkost je tam taky proto, že ji SCD41 počítá z té samé teploty, takže se veze s ní.

**Po probuzení z deep sleepu se vlnovka nezobrazí.** Větev 3V3 se ve spánku nevypíná, takže krabička zůstává tepelně ustálená - ESPHome při bootu jen restartuje periodické měření. Jestli to platí i pro tu první fázi (úvodní skok), je otevřená otázka - viz Known Issues.

> Těch 20 min je extrapolace, ne přímé měření. Ten záznam nebyl studený start (zařízení bylo vypnuté jen pár minut), takže naměřených 12-15 min je spodní mez. Číslo vychází z fitované časové konstanty: BMP180 má τ = 5.9 min a z plných 1.74 °C by na ±0.05 °C potřeboval 20.9 min. Potvrdí se záznamem od opravdu studeného startu.
...
- **SCD41 hlásí po startu měření ~2.5 °C moc - a v CESTA by to mohl být problém (neověřeno).** V záznamu po zapnutí ([`data/2026-08-22-power-on-warmup/`](data/README.md)) startuje teplota SCD41 na 27.5 °C a během ~1.5 min klesne k 25.0. Zajímavé je, že to **nejde vysvětlit teplotou okolí**: zařízení bylo vypnuté jen pár minut, takže box byl prakticky na provozní teplotě, ale samotný čip SCD41 musel být *chladnější* než za provozu. Čip se totiž vlastním teplem drží zhruba 5.6 °C nad vzduchem v krabičce - přesně to kompenzuje `temperature_offset` - takže za běhu sedí kolem 30.6 °C, zatímco po pár minutách bez proudu už stihl chladnout. A vychladlý čip nemůže hlásit vyšší teplotu než rozehřátý. BMP180 přitom dělá pravý opak, startuje na svém minimu a roste. Vypadá to tedy na chování senzoru po `start_periodic_measurement`, ne na teplotu. **Proč to vadí:** ESPHome ten příkaz posílá při každém bootu **včetně probuzení z deep sleepu**, a v režimu CESTA je zařízení vzhůru jen 30 s - celé okno by padlo doprostřed toho skoku a každé probuzení by ukazovalo teplotu o ~2 °C vyšší. Firmware s tím zatím **záměrně nic nedělá**, protože to není ověřené: rozhodne to záznam od opravdu studeného startu (vychladlý čip hlásící vysoko = důkaz, že jde o senzor) a pak test v CESTA přes několik probuzení.
