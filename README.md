...
- **SCD41 hlásí po startu měření ~2.5 °C moc - a v CESTA by to mohl být problém (neověřeno).** V záznamu po zapnutí ([`data/2026-08-22-power-on-warmup/`](data/README.md)) startuje teplota SCD41 na 27.5 °C a během ~1.5 min klesne k 25.0. Zajímavé je, že to **nejde vysvětlit teplotou okolí**: zařízení bylo vypnuté jen pár minut, takže box byl prakticky na provozní teplotě a die SCD41 musel být *chladnější* než jeho ustálených 30.6 °C raw - vychladlý die nemůže hlásit vysoko. BMP180 přitom dělá pravý opak, startuje na svém minimu a roste. Vypadá to tedy na chování senzoru po `start_periodic_measurement`, ne na teplotu. **Proč to vadí:** ESPHome ten příkaz posílá při každém bootu **včetně probuzení z deep sleepu**, a v režimu CESTA je zařízení vzhůru jen 30 s - celé okno by padlo doprostřed toho skoku a každé probuzení by ukazovalo teplotu o ~2 °C vyšší. Firmware s tím zatím **záměrně nic nedělá**, protože to není ověřené: rozhodne to záznam od opravdu studeného startu (vychladlý die hlásící vysoko = důkaz, že jde o senzor) a pak test v CESTA přes několik probuzení.
