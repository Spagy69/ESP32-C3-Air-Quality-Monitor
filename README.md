...
**Po probuzení z deep sleepu se vlnovka nezobrazí** - a je to záměr jen zpola. Že se krabička nemusí znovu zahřívat, platí: ve spánku se ustálí na okolní teplotě. Jenže právě proto je klidový offset v tom režimu odečítaný špatně (**−3.55 °C**, viz Known Issues), a tuhle chybu vlnovka nepokrývá. Nepokrývá ani **chladnutí** po přepnutí z nepřetržitého provozu do spánku, které trvá ~2.5-3 h.
...
| V režimu s uspáváním | **−3.55 °C** (změřeno srpen 2026) |

Ta poslední řádka je **jediná, kterou vlnovka neoznačí** - a od srpna 2026 už není odhad. Offsety jsou změřené pro nepřetržitý provoz, kde se krabička zahřívá pořád; v uspávaném režimu je zařízení vzhůru 4 % času, takže se odečítá teplo, které tam není. Změřeno proti referenčnímu teploměru přes noc ([`data/2026-08-23-cesta-baseline/`](data/README.md)) - podrobně v Known Issues níž.
...
- **V uspávaném režimu je teplota o 3.55 °C pod realitou - změřeno, zatím neopravené (srpen 2026).** Předpověď z minula ("stejný offset tam nejspíš odečítá víc, než by měl") seděla, a teď má číslo. Přes noc v uspávaném režimu s intervalem 10 min ([`data/2026-08-23-cesta-baseline/`](data/README.md), 12.4 h) drží BMP180 posledních pět hodin naprosto klidně **18.955 °C ± 0.043**, zatímco referenční teploměr vedle hlásí **22.5 °C**. Publikovaná chyba je tedy **−3.55 °C**; syrové čtení senzoru (tj. +2.46 zpátky) je na 21.415, tedy −1.09 °C.
  ...
  **Zbytkových −1.09 °C proti syrovému čtení otevřených zůstává** - může to být vlastní chyba BMP180 (datasheet ±1 °C), chyba teploměru (±0.5 °C), nebo skutečný rozdíl mezi místem krabičky a místem teploměru. Jeden odečet, jeden přístroj, neznámé umístění; na novou konstantu to nestačí. Klidový offset v DOMA se dělal ze **tří** odečtů po pěti minutách plus dvou kontrol platnosti a stejný protokol chce i tenhle.

  **Nejhorší na tom je, že se to nijak neoznačí.** Displej zkusí SCD41 → BMP180 → cache; SCD41 stav nemá, takže se vezme BMP180, ten stav **má**, takže `stale` je `false` - a `suspect` je taky `false`, protože po probuzení z deep sleepu není `cold_boot` a nenabíjí se. Vlnovka se neobjeví a v autě je vidět sebevědomé číslo o 3.5 °C vedle.

- **V uspávaném režimu nefunguje žádný filtr, který potřebuje historii - včetně detekce nabíjení (nalezeno srpen 2026, zatím neopravené).** Deep sleep je **plný reboot**: každé probuzení začíná od nuly a trvá ~26 s. Při `update_interval: 60s` z toho plyne, že BMP180, tlak i `battery_voltage` stihnou za probuzení právě **jedno** čtení - a všechno, co se počítá z několika za sebou, tak nemá z čeho počítat.
  ...
  Stejný mechanismus (z konfigurace, ne z těchhle dat) platí i pro medián ze 3 u `battery_voltage` (`batt_v_hist1/2` jsou `restore_value: false`, takže po každém bootu spadne kód do větve `v = raw_v`) a hlavně pro **detekci nabíjení**: `batt_v_slow_avg`, `charge_confirm_count`, `full_confirm_count` i `is_charging` jsou všechny `restore_value: false`. Klouzavý průměr s časovou konstantou ~10 min dostane za probuzení jeden vzorek a pak se zahodí, takže **`Charging` v uspávaném režimu nemůže sepnout nikdy** - a s ním nefunguje ani vlnovka při nabíjení. V autě s USB nabíječkou to znamená, že by teplota byla až o +3.7 °C vyšší a **neoznačená**. V tom záznamu se nenabíjelo, takže to není potvrzené z dat; z konfigurace to ale plyne jednoznačně.

- **`Battery Level` chybí ve 29 % probuzení (nalezeno srpen 2026, zatím neopravené).** V nočním záznamu je `unknown` ve **21 ze 72** probuzení. Komentář v kódu přitom tvrdí, že nastat nemá ("the voltage sensor is registered first and runs first on a shared 60s tick") - nastává skoro v třetině případů. Obě šablony mají `update_interval: 60s` a ESPHome plánuje první běh intervalu na `now + náhodný offset v [0, 5 s)` (`Scheduler::calculate_interval_offset_`), a to **nezávisle pro každou komponentu**. Když padne `battery_level` dřív než `battery_voltage`, uplatní se pojistka `if (!id(battery_voltage).has_state()) return {};`, nepublikuje se nic - a protože je zařízení vzhůru jen ~26 s, **druhá šance v tom probuzení nepřijde**. Displej tím netrpí (`cached_battery_pct` je flash-backed), postižená je řada v Home Assistantu. Čisté řešení je procenta nepočítat vlastním tikem, ale publikovat je z lambdy `battery_voltage`, kde je napětí z definice už k dispozici.
- **BMP180 občas pustí jedno úplně vedlejší čtení (opraveno, srpen 2026).** V hodinovém záznamu (`data/2026-08-22-doma-baseline/`, 01:56:54Z) spadla teplota na 24.41 °C mezi sousedy ~24.70 °C - a **stejné čtení** zároveň ukázalo tlak 956.41 hPa proti sousedním ~957.05 hPa. Jde tedy o jedno špatné I2C čtení, ne o šum na jednom kanálu. BMP180 do té doby neměl žádný filtr, takže se ta hodnota propsala do HA, na displej i do cache. Přidán `median` filtr (okno 3) na teplotu i tlak, zapojený **před** korekční lambdu, aby se čistila i cachovaná hodnota. **Platí to ale jen v nepřetržitém provozu** - v uspávaném režimu ten filtr nedělá nic, protože každé probuzení začíná s prázdným oknem (viz bod o filtrech výš), a v nočním záznamu proto jeden takový výkyv (−0.886 °C) znovu prošel.
