...
| V režimu s uspáváním | **−0.90 °C** (změřeno po opravě; před ní −3.55) |

Ta poslední řádka je od srpna 2026 změřená a **z velké části opravená**. Offsety byly změřené pro nepřetržitý provoz, kde se krabička zahřívá pořád; v uspávaném režimu je zařízení vzhůru 4 % času, takže se odečítalo teplo, které tam není. Firmware ho tam už neodečítá. Změřeno dvakrát proti referenčnímu teploměru: **−3.55 °C** před opravou přes noc ([`data/2026-08-23-cesta-baseline/`](data/README.md)) a **−0.90 °C** po ní ([`data/2026-08-23-cesta-po-oprave/`](data/README.md)) - tedy ne odhad, ale dvě měření na tom samém hardwaru. **Zbytek zavřený není**: −0.90 je pořád dvojnásobek deklarované ±0.5 °C a bude na to potřeba vlastní měření třemi odečty po pěti minutách. Podrobně v Known Issues níž.
...
- ~~V uspávaném režimu je teplota o 3.55 °C pod realitou~~ **z velké části opraveno (srpen 2026).** Předpověď z minula ("stejný offset tam nejspíš odečítá víc, než by měl") seděla, a teď má číslo. Přes noc v uspávaném režimu s intervalem 10 min ([`data/2026-08-23-cesta-baseline/`](data/README.md), 12.4 h) drží BMP180 posledních pět hodin naprosto klidně **18.955 °C ± 0.043**, zatímco referenční teploměr vedle hlásí **22.5 °C**. Publikovaná chyba je tedy **−3.55 °C**; syrové čtení senzoru (tj. +2.46 zpátky) je na 21.415, tedy −1.09 °C.
  ...
  **Zbytkových −0.90 °C tím ale opravené není** a je to dvojnásobek deklarované přesnosti ±0.5 °C. Zavře to až měření třemi odečty po pěti minutách v uspávaném režimu, stejný protokol jako u `rest-offset` - tři body rozházené přes dvě hodiny na novou konstantu nestačí.
