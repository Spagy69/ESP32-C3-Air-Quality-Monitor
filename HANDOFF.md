...
2. Kalibrační konstanty: **teplotní offsety HOTOVO** (viz `2026-08-22-rest-offset` výš, změřeno proti referenci: 6.7 a 2.46). **`CHARGE_OFFSET_*` HOTOVO** - zůstávají 0.0, ale už jako změřené rozhodnutí: teplo z nabíjení je +3.7 °C ve vrcholu, závisí na nabíjecím proudu a žádná konstanta ani časová křivka to nepokryje, takže se to jen značí vlnovkou. **Zbývá už jen dělič napětí** (násobič 3.2 ověřený jen v bodě 4.15 V) - chtělo by to multimetr ještě kolem 3.5 a 3.8 V.
...
2f. ~~Detekce konce nabíjení~~ **HOTOVO (srpen 2026).** Druhé pravidlo vedle trendového: napětí ≥ 4.10 V a změna < 0.001 V po 5 čtení za sebou → `is_charging = false`. Ověřeno simulací nad `data/2026-08-22-charging-from-empty/`: sepne v 18:19, tedy přesně v okně skutečného konce nabíjení (18:10-18:25), o 65 min dřív než 3h pojistka. **Prahy vybrané prohledáním, ne od oka** - při 0.002 V skáče výsledek mezi 17:52 a 18:18 podle jediného kroku v `FULL_CONFIRM` (cliff), při 0.001 V dá jakýkoliv počet 3 až 10 shodně 18:13-18:26. `FULL_V` je prakticky jedno (4.00 až 4.12 dá stejný výsledek), rozhoduje plochost; napěťová podmínka je jen pojistka proti záměně za zaseknuté nabíjení na nízkém napětí. Když by prahy byly na jinou baterku moc těsné, pravidlo nesepne a převezme to 3h backstop - degradace, ne rozbití.
