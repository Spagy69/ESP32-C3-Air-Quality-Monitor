...
- **ZÁZNAM S NABÍJEČKOU (srpen 2026, `data/2026-08-28-cesta-nabijeni-a-chladnuti/`) - 3.5 h, 22 probuzení, zavřel předposlední otevřený hardwarový test.** Baterka dojetá do vypnutí, v 19:28 zapojená nabíječka (zařízení se tím okamžitě nabootovalo - XIAO bere napájení z USB a nečeká na článek), hned přepnuto na CESTA, nabíječka odpojená ve 20:34. Referenční teploměr vedle.
  ...
  - **`Charging` vypne po odpojení: PROŠLO**, 41 min, přesně na tři potvrzení. Vyžádalo si to čtyři probuzení, protože na jednom z nich vydal medián **bit-identickou** hodnotu → krok přesně 0 → větev "ani pokles" → vynulovaný čítač. **Zvažovaná oprava `step <= 0` byla sweepem zamítnuta:** na nabíjení z prázdné baterky vypne příznak o **56 minut dřív** (17:12 místo 18:08), uprostřed ploché CV fáze, kde nulové kroky vznikají i normálním během. To je ten samý typ chyby, proti kterému bylo `< 0` vybrané. **Zůstává `step < 0`.**
...
2. ~~Zkompilovat a otestovat aktuální YAML~~ **ČÁSTEČNĚ** - kompiluje se, běží na hardwaru, menu je proklikané a probuzení tlačítkem ověřené (srpen 2026). Zbývá **CO2 alarm**, který nikdy nesepnul. Pokus ve spánku (`data/2026-08-28-co2-dychnuti-v-cesta/`) selhal jen na množství - jedno dýchnutí dalo 2305 ppm proti prahu 3000, chybělo 695. Zato vyvrátil dvě obavy: krabička drží vydýchaný vzduch s časovou konstantou **~8.9 min** (ne minutu dvě, jak se čekalo), a armovací pravidlo se ve 30s okně **stihne** - pět čtení mělo rozptyl 9 ppm proti prahu stability 150. Viz seznam testů v v2 STATUS.
...
6. ~~Notifikace na nízké napětí v HA~~ **HOTOVO jako dokumentace (srpen 2026)** - hotová automatizace je v `README.md`, sekce "Notifikace na slabou baterku". Do firmwaru nepatří: ESPHome by ji přes `homeassistant.service` poslat uměl, ale to volání potřebuje živé API spojení stejně jako automatizace v HA, takže má tu samou slepou skvrnu - a zadrátovalo by práh do firmwaru. **Slepá skvrna: v opravdové CESTA to nefunguje vůbec**, ten preset vypíná WiFi. Tam by pomohlo jedině lokální varování na displeji, které dnes není - viz nový bod 9.
8. **Hardwarové testy k opravám uspávaného režimu (srpen 2026)** - z principu je nejde ověřit ze stolu, `esphome compile` o nich nic neřekne. **Tři z nich zavřel ověřovací záznam `data/2026-08-23-cesta-po-oprave/`** (2.8 h, 16 probuzení, flashnuto 17:40), čtvrtý zavřela reference v tom samém záznamu, **pátý zavřel `data/2026-08-28-cesta-nabijeni-a-chladnuti/`** (3.5 h, 22 probuzení, nabíječka 19:28-20:34) a šestý je v něm z poloviny hotový:
   ...
   - ~~Chyba proti referenčnímu teploměru~~ **PROŠLO a od 28. 8. i vysvětlené.** −0.90 °C (23. 8., sd 0.18) a −0.94 °C (28. 8., sd 0.02), tedy reprodukovatelné. Reference je starší lihový teploměr s modrou kapalinou, **tentýž kus jako při kalibraci offsetů** - a z toho se dopočítalo, že skutečné self-heating v DOMA je 3.40 °C a konstanta 2.46 byla vždycky rozdíl ohřevu a chyby BMP180. **Dalšími odečty tím samým teploměrem se to nezavře**, jeho chyba se z nich nevydělí; chce to druhý kus.
...
10. **Návrh: vystavit `co2_alarm_active` jako `binary_sensor` do HA.** Dnes je to jen globál - zapne displej, vynutí STAY a vykreslí varovnou obrazovku, ale do HA se nepublikuje. Znamená to, že **sepnutí alarmu nejde ověřit zpětně z dat** ani na něj navázat automatizaci. V `data/2026-08-22-breath-test/` byl práh 3000 ppm překročený na 6.5 minuty (maximum 5038) se zařízením vzhůru v DOMA a s armovacím pravidlem splněným, takže alarm nejspíš sepnul - ale v exportu po tom není ani stopa. **Neimplementováno**, je to nová funkce.
9. **Návrh: lokální varování na slabou baterku na displeji** - v opravdové CESTA je WiFi vypnutá, takže žádné upozornění přes Home Assistant dorazit nemůže a displej je jediný kanál, který zbývá. Dnes na něm ale slabá baterka nijak nevyčnívá: procenta i napětí se kreslí stejným písmem jako při plné (`packages/display.yaml`, kolem ř. 317-329). Nabízí se blikání nebo vlnovka u hodnoty pod nastaveným prahem. **Neimplementováno**, je to nová funkce, ne oprava.
